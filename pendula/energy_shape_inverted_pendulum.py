...
k1 = 0.05 # Controller Gain
k2 = 0.01 # Controller Gain

#||Time and Initial Conditions||
theta_0 = math.radians(60) # rad
theta_dot_0 = 0 # rad/s
t_start = 0 # seconds
t_end = 20 # seconds
...
# Equations of Motion
# -----------------------------
def energy_shape_inverted_pendulum(t, x) -> list[float]:
    theta, theta_dot = x
    tau = theta_dot*b - k1*(0.5*m*(l**2)*(theta_dot**2) - m*g*l*(1+math.cos(theta))) * theta_dot
    # tau = theta_dot*b - k2*(0.5*l*theta_dot - g*(1+math.cos(theta)) ) * theta_dot
    theta_ddot = (tau - theta_dot*b)/(m*l**2) - (math.sin(theta)*g/l)
...
solution = solve_ivp(
    energy_shape_inverted_pendulum, 
    [t_start, t_end],
    [theta_0, theta_dot_0],
    t_eval=t_vals,
    method="RK45", 
